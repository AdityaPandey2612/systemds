...
The project originally had a six-week implementation plan, but after the second TA meeting the schedule is compressed to four active weeks. The TA confirmed that the overall task split is good, but the plan must emphasize test-first development, include both TSMM directions, remove Spark benchmarking from scope, include performance tests in the PR, and open the draft PR early enough for TA feedback.
...
### Updated Scope from Second TA Meeting

| Topic | Plan Update |
| --- | --- |
| Overall plan | TA said the split and plan looked good. |
...
- A draft PR opened by the end of Week 3 for early TA review.
...
**Goal:** Make both operators reviewable, collect first benchmark numbers, and open the draft PR early enough for TA feedback.
...
| Person C: Testing, Benchmarking, Integration | Extend `TransposeSelfMMTest.java`; run focused correctness tests; implement CP-vs-OOC performance tests; collect preliminary benchmark numbers; open draft PR and request TA's free early review. |
...
### Week 4: TA Feedback, Final Benchmarks, and Submission Cleanup

**Goal:** Use TA feedback, finalize benchmarks, clean the PR, and prepare final submission.
...
- TA feedback addressed where possible.
...
| Week 3 | Both operators are correctness-testable; draft PR is open for TA review. |
| Week 4 | TA feedback is addressed; final tests and benchmarks are documented. |
...
Benchmarks should compare CP vs OOC only. Spark benchmarking is intentionally removed from scope after the second TA meeting.
...
- Draft PR is opened by the end of Week 3 for TA feedback.
...
| TA feedback arrives late | Request early review as soon as the Week 3 draft PR is open. |
...
- Open the draft PR early, ideally by the end of Week 3, so the TA can provide one non-grading review before the deadline.
